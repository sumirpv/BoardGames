...
module.exports = function(app) {
    ...
    app.post("/api/boardGames/numPlayers/:numPlayersid", function(req,res) {
        if (req.params.numPlayersid) {
            db.sequelize.query("select * from boardGames as b inner join categories as c on b.category_id = c.id inner join ages as a on b.age_id = a.id where  " + req.params.numPlayersid + " between minPlayer and maxPlayer ", 
            { 
    ...
    app.post("/api/boardGames/timePlay/:timePlayid", function(req,res) {
        if (req.params.timePlayid) {
            db.sequelize.query("select * from boardGames as b inner join categories as c on b.category_id = c.id inner join ages as a on b.age_id = a.id where timeToPlay ='" + req.params.timePlayid + "'", 
            {  
                type: db.sequelize.QueryTypes.SELECT 
            }).then(function(result) {
                res.json(result);
